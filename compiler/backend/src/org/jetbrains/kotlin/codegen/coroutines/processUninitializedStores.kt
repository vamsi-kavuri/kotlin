...
class UninitializedStoresProcessor(private val methodNode: MethodNode, private val forCoroutines: Boolean) {
    ...
    fun run() {
        ...
        for ((index, insn) in methodNode.instructions.toArray().withIndex()) {
            ...
            val initReceiverValue = frame.getInitReceiverValue(insn) ?: continue

            val newInsn = initReceiverValue.newInsn
            val usageInfo = interpreter.uninitializedValueUsageInfo[newInsn]!!
            if (!usageInfo.isUsed) {
                if (forCoroutines)
                    error("At least DUP copy operation expected")
                else
                    continue
            }

            // Value generated by NEW wasn't store to local/field (only DUPed)
            if (usageInfo.storeInsns.isEmpty()) continue

            methodNode.instructions.run {
                removeAll(usageInfo.allUsages)
                if (forCoroutines) {
                    remove(newInsn)
                }
                else {
    ...
    private inner class UninitializedNewValueFrame(nLocals: Int, nStack: Int) : Frame<BasicValue>(nLocals, nStack) {
        override fun execute(insn: AbstractInsnNode, interpreter: Interpreter<BasicValue>?) {
            val initReceiverValue = getInitReceiverValue(insn)

            super.execute(insn, interpreter)

            if (initReceiverValue != null) {
                // New value is initialized now. Replace it across the frame.
                // Usually it is stack top, but in case of some manually generated methods,
                // such as ones generated by android-extensions compiler plugin,
                // it can be anywhere in the stack.
                val newValue = StrictBasicValue(initReceiverValue.type)

                val newStackContent = (0 until stackSize).map {
                    val stackValue = getStack(it)
                    if (stackValue === initReceiverValue) newValue else stackValue
                }

                while (stackSize > 0) pop()
                for (stackValue in newStackContent) push(stackValue)
            }
        }
    }
     ...
     */
    private fun Frame<BasicValue>.getInitReceiverValue(insn: AbstractInsnNode): UninitializedNewValue? {
        ...
        val initParamsCount = Type.getArgumentTypes((insn as MethodInsnNode).desc).size
        val initReceiver = peek(initParamsCount)
        if (forCoroutines) {
            val initReceiverCopy = peek(initParamsCount + 1)
            assert(initReceiverCopy === initReceiver) { "Expected an original uninitialized object under init receiver" }
        }

        return initReceiver as? UninitializedNewValue
    }
    ...
    private class UninitializedNewValueUsageInfo(newInsn: TypeInsnNode, typeInternalName: String) {
        val value = UninitializedNewValue(newInsn, typeInternalName)

        val dupInsns = mutableSetOf<AbstractInsnNode>()
        val storeInsns = mutableSetOf<AbstractInsnNode>()
        val otherCopyInsns = mutableSetOf<AbstractInsnNode>()

        fun add(insn: AbstractInsnNode) {
            when {
                insn.opcode == Opcodes.DUP -> dupInsns.add(insn)
                insn.isStoreOperation() -> storeInsns.add(insn)
                else -> otherCopyInsns.add(insn)
            }
        }

        val isUsed get() = dupInsns.isNotEmpty() || storeInsns.isNotEmpty() || otherCopyInsns.isNotEmpty()
        val allUsages get() = dupInsns + storeInsns + otherCopyInsns
    }

    private class UninitializedNewValueMarkerInterpreter : OptimizationBasicInterpreter() {
        val uninitializedValueUsageInfo = hashMapOf<AbstractInsnNode, UninitializedNewValueUsageInfo>()

        override fun newOperation(insn: AbstractInsnNode): BasicValue? {
            if (insn.opcode == Opcodes.NEW) {
                return uninitializedValueUsageInfo.getOrPut(insn) {
                    UninitializedNewValueUsageInfo(insn as TypeInsnNode, insn.desc)
                }.value
            }
        ...
        override fun copyOperation(insn: AbstractInsnNode, value: BasicValue?): BasicValue? {
            if (value is UninitializedNewValue) {
                assert(insn.opcode != Opcodes.PUTFIELD && insn.opcode != Opcodes.PUTSTATIC) {
                    "Uninitialized value should never be stored in a field"
                }
                uninitializedValueUsageInfo[value.newInsn]!!.add(insn)
                return value
            }
