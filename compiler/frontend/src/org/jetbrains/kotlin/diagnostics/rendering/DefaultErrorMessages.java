...
public class DefaultErrorMessages {
    ...
    static {
        ...
        DiagnosticParameterRenderer<Pair<LanguageVersion, String>> requireLanguageVersionMessage = (pair, renderingContext) -> {
            String message = pair.getSecond();
            return pair.getFirst().getVersionString() + (message != null ? ". " + message : "");
        };
        MAP.put(REQUIRE_LANGUAGE_VERSION_DEPRECATION, "''{0}''{1} should not be used in Kotlin {2}", DEPRECATION_RENDERER,
                (obj, renderingContext) -> obj.equals(RequireLanguageVersion.Version.INFINITY) ? "" : " is only supported since Kotlin " + obj.asString() + " and",
                requireLanguageVersionMessage);
        MAP.put(REQUIRE_LANGUAGE_VERSION_DEPRECATION_ERROR, "''{0}''{1} cannot be used in Kotlin {2}", DEPRECATION_RENDERER,
                (obj, renderingContext) -> obj.equals(RequireLanguageVersion.Version.INFINITY) ? "" : " is only available since Kotlin " + obj.asString() + " and",
                requireLanguageVersionMessage);
